...
class CustomTokenizer(nn.Module):
    ...
    def train_step(self, x_train, y_train, log_loss=False):
        ...
        y_train_hot = y_train_hot.to('cuda').long() ## 이를 long으로 만들어줌
    ...
    def load_from_checkpoint(path, map_location: 'cuda:0'): # map_location : MAP_LOCATION = None
        ...
        with ZipFile(path) as model_zip:
            filesMatch = [file for file in model_zip.namelist() if file.endswith('.info')] # 끝부분 / 없앰
        ...
        if old:
            ...
            print(data_from_model.hidden_size, data_from_model.input_size, data_from_model.output_size, data_from_model.version)
...
def auto_train(data_path, save_path='model.pth', load_model: str | None = None, save_epochs=1):
    ...
    for input_file in os.listdir(ready):
        ...
        try:
            ...
            raise e
        if number == 8000: # 30000개 하려니 RAM이 자꾸 터져서 그냥 중간에 끊음
            break
        if input_file.endswith(sem_string):
            print(f'{number} semantic : {numpy.load(full_path).shape}')
            data_y[number] = numpy.load(full_path)
        elif input_file.endswith(feat_string):
            print(f'{number} feature : {numpy.load(full_path).shape}')
